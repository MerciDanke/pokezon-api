...
namespace :worker do
  namespace :run do
    ...
    task :test => :config do
      sh 'RACK_ENV=test bundle exec shoryuken -r ./workers/search_products_worker.rb -C ./workers/shoryuken_test.yml'
    end

    desc 'Run the background cloning worker in production mode'
    task :production => :config do
      sh 'RACK_ENV=production bundle exec shoryuken -r ./workers/search_products_worker.rb -C ./workers/shoryuken.yml'
    end
  end
end
